...
- [x] `display_screen.dart` — layar utama fullscreen + background foto Masjid Nabawi + gradient overlay + header nama masjid & alamat kiri, jam kanan
...
- [x] `widgets/prayer_status_overlay.dart` — tampilan bersih untuk 3 state (azanAlert blink, iqomah, jamaah)
...
> Layout polish selesai. Tampilan sudah premium — background Masjid Nabawi, card glassmorphism, header nama + alamat masjid, jam proporsional, running text marquee smooth.
> State azanAlert & jamaah belum ditest live. Siap lanjut ke Build & Deploy atau fitur tambahan.
...
- [ ] Test state "Waktunya Sholat [Nama]" muncul & berkedip saat adzan tiba
- [ ] Test transisi otomatis azanAlert → iqomah → jamaah → normal
